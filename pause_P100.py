...
def train(st):
    ...
    num_dataloader_workers = 10 if device.type == 'cuda' else 0
...
def serving(st):
    ...
    docker_command_str = (
        ...
        f"nvcr.io/nvidia/tritonserver:23.05-py3 "
...
Load = 0
Max_Load = 50
# *** MODIFIED: SYS_STATE is no longer needed, replaced by a boolean flag ***
...
# --- Core logic: Process Management (MODIFIED) ---
def manage_processes():
    """
    MODIFIED: Pauses/resumes the training process based on load.
    ...
    with process_management_lock:
        ...
        if not train_is_effectively_running:
            # If train process has died for some reason, do nothing.
            # The main cleanup will handle it.
            return
        ...
        if Load > Max_Load and not train_is_paused:
            try:
                ...
                print(f"MONITOR: Load ({Load}) > Max_Load ({Max_Load}). Paused training process (PID: {train_process.pid}).")
            except ProcessLookupError:
        ...
        # --- Low Load Condition: RESUME Training ---
            ...
            try:
                ...
                train_is_paused = False
                print(f"MONITOR: Load ({Load}) <= Max_Load ({Max_Load}). Resumed training process (PID: {train_process.pid}).")
            except ProcessLookupError:
...
def cleanup_all_processes():
    ...
    with process_management_lock:
        if train_process and train_process.is_alive():
            ...
            try:
                # *** MODIFIED: Ensure process is running before sending SIGINT ***
                if train_is_paused:
                    print(f"MAIN: Resuming paused training process (PID: {train_process.pid}) before termination.")
                    os.kill(train_process.pid, signal.SIGCONT)
                    time.sleep(0.1) # Give it a moment to resume

                print(f"MAIN: Sending SIGINT to training process (PID: {train_process.pid})...")
...
if __name__ == "__main__":
    ...
    # *** MODIFIED: Start BOTH processes at the beginning ***
    print("MAIN: Starting both training and serving processes concurrently...")
    
    # Using multiprocessing.Value for shared memory between processes if needed by the functions
    ...
    train_process.start()
    print(f"MAIN: Training process started (PID: {train_process.pid}).")
    ...
    # Start the monitor thread after processes are launched
    ...
    try:
        ...
        if monitor.is_alive():
            ...
            monitor.join(timeout=5) # Shortened timeout
            if monitor.is_alive():
