...
METRICS_LOG_FILE = "./monitor_metrics.log"
ACTIVE_WORKERS_FILE = "/home/pacs/Kevin/DynGPUs/sys_v100_long_details/active_workers.txt"
...
async def scale_down(count: int) -> bool:
    """
    Scales down gracefully with granular timing logging.
    """
    t_start = time.time()
    ...
    if actual_count <= 0:
        print("\nScale-down skipped: No shared servers available or minimum breached.")
        return False

    servers_to_scale_down = shared_active_servers[:actual_count]
    
    # 1. HAProxy Update Phase
    ...
    reload_haproxy()
    t_haproxy = time.time() # Checkpoint 1

    # 2. Draining and Sleeping Phase
    async with httpx.AsyncClient() as client:
    ...
    t_end = time.time() # Checkpoint 3 (End)
    
    # Calculate durations
    dur_haproxy = t_haproxy - t_start
    dur_drain = t_drain - t_haproxy
    dur_ranks = t_end - t_drain
    total_duration = t_end - t_start

    log_entry = (
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] SCALE_DOWN: Removed {actual_count} server(s). "
        f"Total: {total_duration:.2f}s | "
        f"Steps: [HAProxy: {dur_haproxy:.2f}s] -> [Drain/Sleep: {dur_drain:.2f}s] -> [Rank Update: {dur_ranks:.2f}s]\n"
    )
...
async def scale_up(count: int) -> bool:
    """
    Scales up with granular timing logging.
    """
    t_start = time.time()
    ...
    t_gpu_check = time.time() # Checkpoint 1

    # 2. Wake Up Phase (HTTP requests)
    successfully_woken = []
    ...
    t_wake = time.time() # Checkpoint 2

    # 3. HAProxy Update Phase
    if await update_haproxy_config([s for s in ALL_SERVERS if s['status'] == 'active']):
        ...
        # Calculate durations
        dur_gpu = t_gpu_check - t_start
        dur_wake = t_wake - t_gpu_check
        dur_haproxy = t_end - t_wake
        total_duration = t_end - t_start

        log_entry = (
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] SCALE_UP: Added {len(successfully_woken)} server(s). "
            f"Total: {total_duration:.2f}s | "
            f"Steps: [GPU Check: {dur_gpu:.2f}s] -> [HTTP Wake: {dur_wake:.2f}s] -> [HAProxy: {dur_haproxy:.2f}s]\n"
        )
...
async def autoscaler_task():
    ...
    async with httpx.AsyncClient() as client:
        while True:
            ...
            details_str = " | ".join(server_details)
            ...
            try:
                log_line = (
                    ...
                    f"Active: {len(active_servers_for_metrics)}, "
                    f"SmoothedLoad: {smoothed_avg_load:.2f}, "
                    f"InstLoad: {instantaneous_avg_load:.2f}, "
                    f"TotalLoad: {total_load:.0f}, "
                    f"Details: [{details_str}]\n"
                )
                with open(METRICS_LOG_FILE, "a") as f:
                    f.write(log_line)
            except Exception as e:
                print(f"⚠️ Failed to write to metrics log: {e}")
