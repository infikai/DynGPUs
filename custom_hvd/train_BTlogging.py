...
def main():
    ...
    # MODIFICATION: Set up logging file on rank 0 only
    log_file = None
    log_filepath = '' # Initialize for the final print statement
    if hvd.rank() == 0:
        try:
            os.makedirs(args.log_dir, exist_ok=True)
            log_filepath = os.path.join(args.log_dir, 'batch_times.txt')
            log_file = open(log_filepath, 'w')
            print(f"Logging batch times to {log_filepath}")
        except IOError as e:
            print(f"Error: Rank 0 could not open log file {log_filepath}. Reason: {e}")
            log_file = None
    ...
    try:
            ...
            while True:
                ...
                if hvd.rank() in current_active_ranks:
                    try:
                        ...
                        batch_time = time.time() - ST_batch
                        print(f'One Batch Cost: {batch_time}s')
                        
                        # MODIFICATION: Only rank 0 will have a valid log_file handle and will write.
                        if log_file:
                            log_file.write(f"Epoch: {state.epoch}, Batch: {state.batch_idx}, Time: {batch_time:.4f}\n")
                            log_file.flush()
        ...
        # MODIFICATION: Only rank 0 will have a file to close.
        if log_file:
            log_file.close()
            print(f"Batch times saved to {log_filepath}")
