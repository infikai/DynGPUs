    ...
    def deadlinemissscheduler(self,clock):
        ...
        while len(pass_job_ind)>0:
            ...
            for job_ind in pass_job_ind:
                # if self.tagtask[job_ind] == 0: # for ignoring training jobs
...
#         print(f"Final Cost ={self.cost}")
    ...
    def schedule(self):
        ...
        # days = []
        # for i in range(len(self.start_time[1200000:])):
        #     days.append((self.start_time[1200000+i]-self.start_time[0])/3600/24)
        # plt.plot(np.array(days))
        # plt.savefig("thresh.pdf",dpi=300)
        # exit()
        ...
        self.clock = tic_svc(cur_time=self.task_list[0]['start_time'],delta=10) # create universal clock
        self.clock_train = tic_svc(cur_time=self.task_list[0]['start_time'], delta=10) # create universal clock
