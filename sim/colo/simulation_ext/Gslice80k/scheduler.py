...
class scheduler:
    def __init__(self, alloc_policy=0,\
        ...
        self.max_no_gpu_infer=int(self.max_no_gpus/2)
        self.max_no_gpu_train=int(round(self.max_no_gpus/2))
        self.no_gpu_train=1
        self.no_gpu_infer=1
...
###Listing all the GPUs
    ...
    def tic_clock_job(self, clock):
        ...
        for qqk in range(self.no_gpu_infer):
            ...
            if round(clock.get_cur_time(),0) % self.control_per == 0:
                self.obj_infer[qqk].invoke_control()
        ...
        if round(clock.get_cur_time(),0) % 1 == 0:
            ...
            # used_gpu_a = used_gpu_num_infer*6
            # used_gpu_b = used_gpu_num_train//5+used_gpu_num_train
    ...
    def deadlinemissscheduler(self,clock):
        ...
        # for job_ind in range(self.no_jobs):
        #     schedule=False
        #     while clock.get_cur_time() < self.task_list[job_ind]['start_time']:

               
#                 scheduled.append(i) # store the missed jobs
#             if((i%100==0 and len(scheduled)>=1) or (i==self.no_jobs-1 and len(scheduled)>=1)):
#                 #print(scheduled)
#                 while(len(scheduled) > 0):
#                     self.gpu_count()
#                     while(True):
#                         ch=self.missline(scheduled[0])
#                         if ch:
#                             break
#                         # print(ch)
#                         sum=0
...
#                             print("***STime %.2fs, Loop %.2fs, GPUs %d/%d, Finished %d/%d, Train %d, Infer %d, MISS %d***"%(clock.get_cur_time()-self.start_time[0],end_t-start_t,used_gpu_num,self.no_gpus,sum,self.no_jobs, sum_train, sum_infer, len(scheduled)))
#                     scheduled.pop(0)
                
                # if(infersum==self.inferencecount_trace):
                #     print(f"Inter Execution time ={clock.get_cur_time()*10**3} milli seconds")
                #     interimtime=(interimtime+clock.get_cur_time())*10**3
...
#         print(f"Final Cost ={self.cost}")
    ...
    def schedule(self):
        ...
        self.task_list = sorted(self.task_list, key=lambda d: (d['start_time'], -d['utilization'])) # sort ascending order based on start time
        ...
        for i in range(len(self.task_list)):
            ...
            if self.task_list[i]['utilization']==0:
                print("task %d"%i)
                exit()
        self.first_job_time=0
        ...
        self.clock = tic_svc(cur_time=self.task_list[0]['start_time'],delta=10.0) # create universal clock
        self.obj_train = [GPU(memory=self.gpumemory[0],\
                         ...
                         clock=self.clock,\
                             ddl=self.ddl,\
                             slo_factor=self.slo_factor) for i in range(self.no_gpu_train)]
        ...
        self.obj_infer = [GPU(memory=self.gpumemory[0],\
                         ...
                         clock=self.clock,\
                             ddl=self.ddl,\
                             slo_factor=self.slo_factor) for i in range(self.no_gpu_infer)]
        ...
        if(self.deadline==0):
            ...
            for i,gpu in enumerate(self.obj_infer):
