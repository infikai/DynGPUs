...
if __name__ == '__main__':
    ...
    # Let rank 0 create the dataset and broadcast it to other ranks.
    if hvd.rank() == 0:
        ...
        print(f'Tdataset Time on rank 0: {time.time() - start_Tdataset}s')
    else:
        train_dataset = None

    # Broadcast the dataset object from rank 0 to all other ranks.
    start_broadcast_train = time.time()
    train_dataset = hvd.broadcast_object(train_dataset, root_rank=0, name='train_dataset')
    if hvd.rank() == 0:
        print(f'Broadcast train_dataset Time: {time.time() - start_broadcast_train}s')

    # Create and broadcast sampler from rank 0.
    # WARNING: This is not a standard practice and will cause all ranks to use the same data.
    if hvd.rank() == 0:
    ...
    start_broadcast_val = time.time()
    val_dataset = hvd.broadcast_object(val_dataset, root_rank=0, name='val_dataset')
    if hvd.rank() == 0:
        print(f'Broadcast val_dataset Time: {time.time() - start_broadcast_val}s')

    # Create and broadcast validation sampler from rank 0.
    if hvd.rank() == 0:
        val_sampler = hvd.elastic.ElasticSampler(val_dataset)
    else:
        val_sampler = None
    val_sampler = hvd.broadcast_object(val_sampler, root_rank=0, name='val_sampler')

    # Create and broadcast validation loader from rank 0.
    if hvd.rank() == 0:
        ...
        val_loader = None
    val_loader = hvd.broadcast_object(val_loader, root_rank=0, name='val_loader')
