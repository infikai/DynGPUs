...
# --- MODIFICATION START ---
parser.add_argument('--log-file', default='batch_times.log',
                    help='file to log batch times to')
# --- MODIFICATION END ---
        ...
        if hvd.rank() == 0:
            ...
            if idx % 1 == 0:
                print(f'Epoch: [{epoch + 1}][{idx}/{len(train_loader)}]\t'
                      f'Loss {loss.item():.4f}\t')
                print(f'Batch time: {batch_time:.4f}s; Allreduce time: {end_batch - start_op:.4f}s')

            # --- MODIFICATION START ---
            # Log the batch time to the file
            if log_file:
                log_file.write(f'Epoch: {epoch + 1}, Batch: {idx}, Time: {batch_time:.4f}\n')
...
if __name__ == '__main__':
    ...
    # --- MODIFICATION START ---
    # Open the log file for writing on the root worker.
    if hvd.rank() == 0:
    ...
    try:
        ...
        # Ensure the log file is closed on the root worker
        if hvd.rank() == 0 and log_file:
            log_file.close()
            print(f"Batch times successfully logged to {args.log_file}")
    # --- MODIFICATION END ---