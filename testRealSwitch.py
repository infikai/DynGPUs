...
try:
    ...
    # Define dummy classes if PyTorch is not available to prevent NameErrors later if PYTORCH_AVAILABLE is checked
    if 'Dataset' not in globals(): # Check if Dataset was imported from torch.utils.data
        class Dataset: pass 
    if 'ImageFolder' not in globals():
...
def train():
    ...
    model = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1 if PYTORCH_AVAILABLE else None) 
    model.to(device)
