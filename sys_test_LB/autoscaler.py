...
ALL_SERVERS = [
    ...
    {"host": "localhost", "port": 8000, "status": "active", "rank": 2, "shared": True},
    {"host": "localhost", "port": 8001, "status": "sleeping", "rank": 0, "shared": True},
    {"host": "localhost", "port": 8002, "status": "sleeping", "rank": 5, "shared": True},
]
...
async def check_gpu_memory_is_free(server: Dict) -> bool:
    ...
    while (time.time() - start_time) < GPU_FREE_TIMEOUT_SECONDS:
        try:
            async with asyncssh.connect(server['host'], username='pacs') as conn:
