    ...
    async with httpx.AsyncClient() as client:
        while True:
            ...
            if (time.time() - last_scaling_time) > SCALING_COOLDOWN_SECONDS:
                ...
                if smoothed_avg_load < SCALE_DOWN_THRESHOLD and total_load / (len(active_servers_for_metrics)-1 if len(active_servers_for_metrics) > 1 else 1 < SCALE_UP_THRESHOLD:
