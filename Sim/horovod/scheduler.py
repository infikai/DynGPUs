...
class Scheduler:
    ...
    def _dispatch_training_job(self, job):
        """
        Schedules a training job using the V2.1 policy:
        1. NO borrowing from the inference pool.
        2. CAPPED GREEDY allocation: takes minimum needed + up to 1x more (max 2x total).
        3. PREEMPTION of other 'greedy' training jobs if minimum GPUs are not met.
        """
        # 1. Calculate the job's *minimum* GPU requirement
        gpus_needed = max(math.ceil(job.memory_required / GPU_MEMORY_GB),
                          math.ceil(job.utilization_required / GPU_UTILIZATION_PERCENT), 1)
        job.gpus_needed = gpus_needed
        
        # --- NEW: Calculate the *maximum* GPUs this job is allowed to take ---
        max_gpus_allowed = gpus_needed * 2

        # 2. Find all currently idle GPUs in the training pool
        available_gpus = self.cluster.find_all_idle_gpus_in_training_pool()
        
        gpus_still_needed = gpus_needed - len(available_gpus)

        # 3. If minimum is NOT met, preempt 'greedy' jobs in the training pool
        if gpus_still_needed > 0:
            preempted_gpus = []
            for _ in range(gpus_still_needed):
                # Find a 'greedy' job in the training pool to take one GPU from
                victim_job, victim_gpu = self.cluster.find_preemptible_job_in_training_pool(self.clock.current_time)
                
                # Ensure we found a victim
                if victim_job and victim_gpu:
                    victim_job.preempt_and_pause(victim_gpu, self.clock.current_time)
                    # Add to preemption map so the victim can reclaim this GPU later
                    self.preemption_map[victim_gpu.gpu_id] = victim_job
                    self.preemption_count += 1
                    preempted_gpus.append(victim_gpu)
                else:
                    ...
                    for gpu in preempted_gpus:
                         reclaiming_job = self.preemption_map.pop(gpu.gpu_id)
                         # Give the GPU back to the victim job
                         if reclaiming_job in self.running_jobs:
                            reclaiming_job.reclaim_gpu(gpu, self.clock.current_time)
                         self.preemption_count -= 1
                    return False # Job fails to schedule, will be retried
            
            # Add the newly freed GPUs to our list
            available_gpus.extend(preempted_gpus)

        # 4. If we are here, we have met the minimum requirement.
        #    len(available_gpus) is >= gpus_needed.
        if len(available_gpus) >= gpus_needed:
            # --- NEW: Apply the cap ---
            # Determine the final number of GPUs to assign
            num_gpus_to_assign = min(len(available_gpus), max_gpus_allowed)
            
            # Get the actual GPU objects
            gpus_to_assign = available_gpus[:num_gpus_to_assign]
