...
class Scheduler:
    ...
    def _apply_adaptive_llm_policy_individual(self):
        """
        Periodically adjusts the number of LLM servers FOR EACH MODEL
        based on the current load, adjusting ONE GPU AT A TIME.
        """
        
        print(f"--- ⚙️ Clock {self.clock.current_time}: Running adaptive LLM policy (Individual) ---")
        
        # 1. Measure current LLM job load (running + retrying) PER MODEL
        llm_jobs_by_model = {}
        all_active_jobs = list(self.running_jobs) + list(self.jobs_to_retry)
        
        for job in all_active_jobs:
            if job.job_type == 'llm_inference':
                model = job.model_type
                if model not in llm_jobs_by_model:
                    llm_jobs_by_model[model] = 0
                llm_jobs_by_model[model] += 1
        
        # Get a set of all models we need to manage
        all_model_types = set(llm_jobs_by_model.keys())
        
        # 2. Loop through EACH model and apply the policy
        for model in all_model_types:
            llm_jobs_count = llm_jobs_by_model.get(model, 0)
            
            # A. Calculate target for this model
            base_target_gpus = math.ceil(llm_jobs_count / LLM_MAX_CONCURRENCY) + 1
            # ** MODIFIED: Use the exact target **
            target_llm_gpus = base_target_gpus
            
            print(f"    Policy (Model {model}): Found {llm_jobs_count} jobs. Target servers: {target_llm_gpus}")

            # B. Get current servers for this model
            current_llm_gpus = [gpu for gpu in self.cluster.inference_gpus if gpu.llm_model_type == model]
            gpus_to_change = target_llm_gpus - len(current_llm_gpus)

            if gpus_to_change > 0: # --- Scale UP for this model ---
                
                print(f"    Policy (Model {model}): Scaling UP by {gpus_to_change} servers.")
                num_converted = 0

                # Step 1: Find idle NON-SHARABLE GPUs
                candidates_nonsharable = [gpu for gpu in self.cluster.inference_gpus if gpu.is_idle()]
                candidates_nonsharable.sort(key=lambda gpu: gpu.sharable) # Non-sharable first
                
                for gpu in candidates_nonsharable:
                    if num_converted >= gpus_to_change: break
                    gpu.convert_to_llm_server(model) # <-- Pass the model
                    num_converted += 1

                # ** MODIFIED: Preempt training jobs one-by-one **
                gpus_still_needed = gpus_to_change - num_converted
                if gpus_still_needed > 0:
                    print(f"    Policy (Model {model}): Preempting {gpus_still_needed} jobs individually.")
                    
                    for _ in range(gpus_still_needed):
                        # Use the 1-by-1 preemption function from cluster_manager.py
                        ...
                        if victim_job and victim_gpu:
                            ...
                            victim_gpu.convert_to_llm_server(model) # <-- Pass the model
                            num_converted += 1
                        else:
                            print(f"    Policy (Model {model}): No more victims to preempt.")
                            break # Stop trying to preempt
            
            elif gpus_to_change < 0: # --- Scale DOWN for this model ---
                
                print(f"    Policy (Model {model}): Scaling DOWN by {abs(gpus_to_change)} servers.")
                ...
                print(f"    Policy (Model {model}): Reverting {len(gpus_to_revert)} servers.")

                for gpu_to_revert in gpus_to_revert:
                    if gpu_to_revert.running_tasks:
                        print(f"    Policy: Evicting from busy server {gpu_to_revert.gpu_id}...")
    ...
    def _batch_dispatch_llm_jobs(self, llm_jobs):
        """
        Dispatches a batch of LLM jobs by placing them onto the
        correct model-specific servers created by the adaptive policy.
        """
        if not llm_jobs:
            return []

        # 1. Group incoming jobs by their model type
        jobs_by_model = {}
        for job in llm_jobs:
            model = job.model_type
        ...
        # Get all available LLM servers (busy and empty)
        all_llm_servers = [gpu for gpu in self.cluster.inference_gpus if gpu.is_llm_server]
        
        assigned_count = 0
        
        # 2. Loop through each model group and schedule them
        for model_type, jobs in jobs_by_model.items():
            
            # Find *only* servers for this model with free slots
            model_servers = [
                gpu for gpu in all_llm_servers 
                if gpu.llm_model_type == model_type and gpu.llm_slots_available > 0
            ]
            
            model_servers.sort(key=lambda gpu: (gpu.sharable, gpu.llm_slots_available))
            
            job_index = 0
            num_jobs_to_assign = len(jobs)

            # 3. Fill the available GPUs
            for gpu in model_servers:
                if job_index >= num_jobs_to_assign:
                    break # All jobs for this model are assigned
                
                # This server is already the correct model, just get its free slots
                ...
                for _ in range(slots_to_fill):
                    ...
                    gpu.assign_llm_task(job) # This just assigns the task
                    self.running_jobs.append(job)
                    
                    assigned_count += 1
                    job_index += 1
        
        # 4. Return all unassigned jobs from all groups
        # These will wait in the retry queue until the policy creates servers for them.
    ...
    def _dispatch_training_job(self, job):
        ...
        if len(allocated_gpus) == gpus_needed:
            ...
            if not is_high_memory_job:
                extra_gpus_to_request = math.ceil(gpus_needed * 0.8)
    ...
    def run_simulation(self):
        ...
        print("--- ⚙️ Clock 0: Running initial adaptive LLM policy ---")
