...
class GPU:
    ...
    def convert_to_llm_server(self, model_type):
    ...
    def assign_llm_task(self, job):
        """Assigns an LLM task to this GPU (assumes it's the correct model)."""
        if not self.is_llm_server:
            raise Exception(f"CRITICAL ERROR: Scheduler tried to assign LLM job to non-server GPU {self.gpu_id}")
        if self.llm_slots_available <= 0:
            raise Exception(f"Attempted to assign LLM job to a full GPU {self.gpu_id}")
        
        # This is now just a safety check, not a setting operation
        if self.llm_model_type != job.model_type:
            raise Exception(f"Model mismatch on GPU {self.gpu_id}: Server is {self.llm_model_type}, Job is {job.model_type}")
    ...
    def release_task(self, job):
        """Releases a finished task (job) from this GPU."""
        ...
        if task_info.get('type') == 'llm_inference':
            self.llm_slots_available += 1
            # --- REMOVED ---
            # The server STAYS as its model type, even when empty.
            # The adaptive policy will revert it if needed.
        else: # Regular job
...
class Job:
    def __init__(self, id, job_type, arrival_time, base_duration=0, 
                 ...
                 input_tokens=0, output_tokens=0, model_type=None):
        self.id = id
