...
class Scheduler:
    ...
    def _apply_adaptive_llm_policy_chunks(self):
        """
        Periodically adjusts the number of LLM servers FOR EACH MODEL
        based on the current load.
        
        - Target # of GPUs is rounded UP to the nearest 8.
        - Preemption is done one-by-one to meet that target.
        """
        
        print(f"--- ⚙️ Clock {self.clock.current_time}: Running adaptive LLM policy (Chunks of 8 Target) ---")
        ...
        for model in all_model_types:
            ...
            # ** KEY LOGIC: Round target up to nearest 8 **
            target_llm_gpus = math.ceil(base_target_gpus / 8) * 8 
            
            print(f"    Policy (Model {model}): Found {llm_jobs_count} jobs. Base target: {base_target_gpus}, Rounded target: {target_llm_gpus}")
            ...
            if gpus_to_change > 0: # --- Scale UP for this model ---
                ...
                # ** MODIFIED: Preempt training jobs one-by-one using find_preemptible_job **
                gpus_still_needed = gpus_to_change - num_converted
                if gpus_still_needed > 0:
                    print(f"    Policy (Model {model}): Preempting {gpus_still_needed} jobs individually to meet chunk target.")
                    
                    for _ in range(gpus_still_needed):
                        # Use the 1-by-1 preemption function
