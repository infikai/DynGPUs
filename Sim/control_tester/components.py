...
LLM_POLICY_INTERVAL = 30

# LLM Inference Performance Model Constants
LLM_BASE_TTFT = 2.5          
LLM_TKN_PER_INPUT = 0.005    
LLM_TPOT = 0.1               
LLM_MAX_CONCURRENCY = 16     # "Soft" limit for ideal performance
LLM_HARD_CONCURRENCY_LIMIT = 20 # "Hard" limit for server capacity
...
class GPU:
    ...
    def is_available_for_llm(self, current_time):
        # Allow over-provisioning up to a hard limit
        if not self.is_llm_server or len(self.running_tasks) >= LLM_HARD_CONCURRENCY_LIMIT:
            return False
        # Respect drain time
...
class Job:
    ...
    def update_progress(self, time_delta, current_time):
        ...
        work_to_do = 0
        if self.job_type == 'llm_inference':
            gpu = self.assigned_gpus[0]
            num_jobs_on_gpu = len(gpu.running_tasks)
            
            # Performance degrades if more jobs are running than the "soft" concurrency limit
            if num_jobs_on_gpu > LLM_MAX_CONCURRENCY:
                slowdown_factor = LLM_MAX_CONCURRENCY / num_jobs_on_gpu
                work_to_do = time_delta * slowdown_factor
            else:
                work_to_do = time_delta # Ideal performance
        else: # Training job logic
            if self.gpus_needed > 0:
                speedup_factor = len(self.assigned_gpus) / self.gpus_needed
            else:
                speedup_factor = 1 
            work_to_do = time_delta * speedup_factor

        self.remaining_work -= work_to_do
