...
# [MODIFIED] Defined the fixed prompt here. 
# Edit this string to change your input token length.
FIXED_PROMPT_TEXT = (
...
def load_trace_file(filepath: str) -> List[Request]:
    ...
    print(f"[INFO] Overriding all prompts with fixed text (Length: {len(FIXED_PROMPT_TEXT)} chars) to maintain consistent input length.")
            ...
            try:
                ...
                requests.append(
                    Request(
                        ...
                        # [MODIFIED] Ignore the prompt in the file, use the fixed global prompt
...
async def process_request(request: Request, api_url: str, model_name: str) -> RequestResult:
    ...
    payload = {
        "model": model_name,
        "prompt": request.prompt,  # This will now always be FIXED_PROMPT_TEXT
    ...
    for stage_name, rps in stages.items():
        ...
        for req in stage_requests:
            ...
            new_req = Request(
                ...
                prompt=req.prompt, # This will copy the FIXED_PROMPT_TEXT
